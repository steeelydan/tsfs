...
export const checkEnv = (requiredEnvVars: EnvVars | undefined, validEnvValues: ValidEnvValues | undefined): void => {
        ...
        if (!envValue) {
            ...
            if (Array.isArray(validEnvValue)) {
                if (!validEnvValue.includes(envValue)) {
                    throw new Error(
                        'Env var has wrong value: ' + variable + '; must be on of: ' + validEnvValue.join(', ')
                    );
                }
