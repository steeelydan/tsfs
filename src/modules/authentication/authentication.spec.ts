...
describe('Authenticate user', () => {
    it('does not authenticate user with wrong password', async () => {
        ...
        const response = await supertest(app).post('/login').send({ username: 'adminuser', password: '1234' });
    ...
    it('does not authenticate user if user is not found', async () => {
        ...
        const response = await supertest(app).post('/login').send({ username: 'nouser', password: '123' });
    ...
    it('authenticates user with correct password', async () => {
        ...
        const response = await supertest(app).post('/login').send({ username: 'adminuser', password: '123' });
    ...
    it('does not authenticate if cookie is wrong', async () => {
        ...
        const loginResponse = await supertest(app).post('/login').send({ username: 'adminuser', password: '123' });
        expect(loginResponse.statusCode).toBe(200);
        const validCookie = loginResponse.headers['set-cookie'][0].split(';')[0];
    ...
    it('puts a valid user object onto the request', async () => {
        ...
        const response = await supertest(app).get('/').set('Cookie', validCookie);
...
describe('Consider user roles', () => {
    it('logs in non-admin user', async () => {
        ...
        const loginResponse = await supertest(app).post('/login').send({ username: 'nonadminuser', password: 'cba' });
        expect(loginResponse.statusCode).toBe(200);
