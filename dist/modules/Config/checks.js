...
export const checkPublicDir = (tsfsPathConfig) => {
    checkIfPathExistsAndIsAbsolute(tsfsPathConfig.publicDirPath, 'Public Dir');
    if (tsfsPathConfig.publicDirPath && !fs.existsSync(tsfsPathConfig.publicDirPath)) {
        throw new Error('Public directory does not exist. Did you forget to build the client?');
    }
};
export const checkEnv = (requiredEnvVars, validEnvValues) => {
    ...
    Object.keys(validEnvValues).forEach((variable) => {
        ...
        if (!envValue) {
            throw new Error('Variable from validEnvValues does not exist in your .env');
        }
        else {
            if (Array.isArray(validEnvValue)) {
                if (!validEnvValue.includes(envValue)) {
                    throw new Error('Env var has wrong value: ' + variable + '; must be on of: ' + validEnvValue.join(', '));
                }
            }
            else if (typeof validEnvValue === 'function') {
                if (!validEnvValue(envValue)) {
                    throw new Error('Env var has wrong value: ' + variable + '.');
                }
            }
        }
    });
